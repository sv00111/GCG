...
public class LVerb {
	...
	public String generate() {
		...
		switch(rule) {
		case "1":
			str = pick(GCG.lverbList) + adv.generate() + " " + pick(GCG.adjList);
			GCG.objectNotAllowed = true;
			break;
		case "2":
			str = adv.generate() + " " + pick(GCG.lverbList) + " " + pick(GCG.adjList);
			GCG.objectNotAllowed = true;
			break;
		case "3":
			str = pick(GCG.lverbList) + adv.generate() + prep.generate();
			GCG.objectRequired = true;
			break;
		case "4":
			str = adv.generate() + " " + pick(GCG.lverbList) + prep.generate();	
			GCG.objectRequired = true;
			break;
		}
		return str;
